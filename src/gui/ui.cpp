...
void UIRenderer::renderUI(ImGuiParams& gui_params, RayTracer& raytracer, GLResources& gl_res) {
  bool controlsEnabled = (raytracer.status == RayTracer::IDLE);
  ImGui::Begin("Control Panel");

  ImGui::Checkbox("Auto Render", &gui_params.auto_render);
  ...
  if (ImGui::BeginCombo("Scene", Scene::SceneNames[gui_params.current_scene].c_str())) {
    for (int n = 0; n < Scene::SceneNames.size(); n++) {
      ...
      if (ImGui::Selectable(Scene::SceneNames[n].c_str(), is_selected)) {
        ...
        raytracer.setupScene(gl_res, gui_params.current_scene, gui_params.lookFrom, gui_params.lookAt);
        raytracer.renderSceneAsync(gl_res);
      }
